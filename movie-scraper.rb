...
class Movie_Scraper
  ...
  def self.namescrape #div.panel.each do
    ...
    page.css('div.panel').collect do |mov| #moov
      ...
      movie.name= mov.css('h4.media-heading a').text.strip.split("  ")#, page.css('p span.label').text.strip  #grabs names.to_a ["Harry Potter", "Serenity"]
      ...
      movie.rating = mov.css('ins.mpaa').to_s.scan(/\d+/) #(mpaa rating)
    #self.all
    movie
    end
  end
